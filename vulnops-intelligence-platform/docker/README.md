...
| File | Image |
|------|--------|
| [Dockerfile.api](Dockerfile.api) | FastAPI + Alembic entrypoint; non-root user `vulnops` (UID 10001). |
...
```bash
docker build -f docker/Dockerfile.api -t vulnops-api:local .
docker build -f docker/Dockerfile.frontend -t vulnops-web:local .
```
