...
| File | Operator |
|------|----------|
| [external-secret-api.yaml](external-secret-api.yaml) | [External Secrets Operator](https://external-secrets.io/) — syncs `DATABASE_URL` and `JWT_SECRET_KEY` into `vulnops-api-secret`. |
...
3. Seal it:
   ...
   ```bash
   kubeseal -f /tmp/secret.yaml -w sealed-vulnops-api-secret.yaml
   ```

4. Commit **`sealed-vulnops-api-secret.yaml`** only; apply with `kubectl apply -f sealed-vulnops-api-secret.yaml`.
