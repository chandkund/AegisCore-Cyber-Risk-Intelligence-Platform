...
$env:DATABASE_URL = "postgresql+psycopg://USER:PASS@localhost:5432/vulnops"
...
`pytest -m integration` when `VULNOPS_TEST_DATABASE_URL` points at a migrated, seeded PostgreSQL instance.
...
- **PostgreSQL:** `localhost:${POSTGRES_PUBLISH_PORT:-5432}` (user/password/db default `vulnops` / see compose file).  
