...
- **Decision:**
  1. **Docker Compose** at repository root runs **PostgreSQL 16**, **FastAPI** (Alembic on entrypoint, then Uvicorn), and **Next.js** (standalone build). Images run as **non-root** UIDs (`vulnops` / `nextjs`).
