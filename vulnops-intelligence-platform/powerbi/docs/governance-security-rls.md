...
|-------|---------------------|----------|
| Dataset `VulnOps Reporting` | `VulnOps — Production` | Certified; scheduled refresh |
| Report `VulnOps — Executive Risk` | Same | Leadership, risk committee |
| Dev copies | `VulnOps — Dev` | Builders only |
...
Role `Analyst_All` with no table filters; assign to security group `SG-VulnOps-Analysts`.
